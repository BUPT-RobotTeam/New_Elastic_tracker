...
{
    if (waypoints.empty()) {
        ROS_ERROR("Waypoints cannot be empty!");
        return false;
    }
    ...
    path_points.insert(path_points.end(), waypoints.begin(), waypoints.end());

    // 运动学约束
    double max_vel = pp_.max_vel_;
    double max_acc = pp_.max_acc_;
    double min_segment_time = 0.1;

    // 计算每段持续时间和速度（增强段间平滑性）
    std::vector<double> segment_durations;
    std::vector<Eigen::Vector3d> segment_velocities;

    for (size_t i = 0; i < path_points.size() - 1; ++i) {
        Eigen::Vector3d delta = path_points[i+1] - path_points[i];
        double distance = delta.norm();
        if (distance < 1e-3) {
            segment_durations.push_back(0.0);
            segment_velocities.push_back(Eigen::Vector3d::Zero());
            continue;
        }

        Eigen::Vector3d dir = delta.normalized();

        // 段间速度平滑约束：当前速度与前一段速度的变化量不超过 max_acc * 时间
        Eigen::Vector3d prev_vel = (i == 0) ? start_vel : segment_velocities[i-1];
        double prev_vel_proj = prev_vel.dot(dir);  // 前向速度投影
        double max_delta_vel = max_acc * min_segment_time;  // 允许的最大速度变化
        double max_possible_vel = std::min({max_vel, prev_vel_proj + max_delta_vel});  // 上限
        max_possible_vel = std::max(max_possible_vel, prev_vel_proj - max_delta_vel);  // 下限（避免反向突变）
        max_possible_vel = std::max(max_possible_vel, 0.1);  // 最小速度

        // 计算段时间（确保速度合理）
        double duration = distance / max_possible_vel;
        duration = std::max(duration, min_segment_time);

        // 最终速度（沿方向向量，确保匀速）
        Eigen::Vector3d vel = dir * (distance / duration);

        segment_durations.push_back(duration);
        segment_velocities.push_back(vel);
    }

    // 构造轨迹（修正多项式系数列索引）
    poly_traj::Trajectory straight_traj;
    straight_traj.clear();

    for (size_t i = 0; i < path_points.size() - 1; ++i) {
        double duration = segment_durations[i];
        if (duration < 1e-3) continue;

        Eigen::Vector3d p0 = path_points[i];       // 起点位置（t⁰项）
        Eigen::Vector3d vel = segment_velocities[i];  // 匀速速度（t¹项）

        // 五次多项式系数矩阵（列0:t⁵, 列1:t⁴, 列2:t³, 列3:t², 列4:t¹, 列5:t⁰）
        poly_traj::CoefficientMat coeff_mat = Eigen::Matrix3Xd::Zero(3, 6);
        coeff_mat.col(4) = vel;       // t¹项系数 = 速度（匀速）
        coeff_mat.col(5) = p0;        // t⁰项系数 = 起点位置
        // 列4和列5同时存储起点速度和位置（适配框架的衔接逻辑）
        // （与轨迹计算的系数一致，确保节点处状态正确）

        straight_traj.emplace_back(duration, coeff_mat);
    }

    // 验证轨迹合法性
    if (!straight_traj.checkMaxVelRate(max_vel) || !straight_traj.checkMaxAccRate(max_acc)) {
        ROS_WARN("Trajectory exceeds dynamic constraints! Adjusting...");
        return false;
    }

    // 存储轨迹
    auto time_now = ros::Time::now();
    traj_.setGlobalTraj(straight_traj, time_now.toSec());
